...
$(document).ready(function () {
    ...
    if ($("table.sortable_simple").length > 0)$("table.sortable_simple").dataTable({iDisplayLength: 5, bLengthChange: false, bFilter: false, bInfo: false, bPaginate: true});
    if ($("table.sortable_default").length > 0)$("table.sortable_default").dataTable({iDisplayLength: 5, sPaginationType: "full_numbers", bLengthChange: false, bFilter: false, bInfo: false, bPaginate: true, aoColumns: [
        {bSortable: false},
        null,
        null,
        null,
    ...
    if ($("table.sortable").length > 0)$("table.sortable").dataTable({iDisplayLength: 5, aLengthMenu: [5, 10, 25, 50, 100], sPaginationType: "full_numbers", aoColumns: [
        {bSortable: false},
        null,
        null,
        null,
    ...
    if ($(".sparkline").length > 0)$(".sparkline span").sparkline("html", {enableTagOptions: true});
    if ($(".cle").length > 0)cE = $(".cle").cleditor({width: "100%", height: 230});
    if ($(".scle").length > 0)cEC = $(".scle").cleditor({width: "100%", height: 230, controls: "bold italic underline strikethrough link unlink"})[0].focus();
    $("#modal_mail").on("shown.bs.modal", function () {
        cEC.refresh()
    });
    if ($("#validate").length > 0 || $("#validate_custom").length > 0)$("#validate, #validate_custom").validationEngine("attach", {promptPosition: "topLeft"});
    if ($("#wizard").length > 0)$("#wizard").stepy();
    if ($("#wizard_validate").length > 0) {
        $("#wizard_validate").validationEngine("attach", {promptPosition: "topLeft"});
        $("#wizard_validate").stepy({back: function (e) {
        }, next: function (e) {
    ...
    if ($(".fancybox").length > 0)$(".fancybox").fancybox({padding: 10});
    ...
    if ($("#calendar").length > 0) {
        ...
        var i = $("#calendar").fullCalendar({header: {left: "prev,next today", center: "title", right: "month,agendaWeek,agendaDay"}, editable: true, events: [
        ...
        ], droppable: true, selectable: true, selectHelper: true, select: function (e, t, n) {
    ...
    $("#slider").slider({range: "min", min: 0, max: 100, value: 50});
    $("#slider2").slider({range: true, min: 0, max: 500, values: [150, 350]});
    $("#slider3").slider({orientation: "vertical", range: "min", min: 0, max: 100, value: 50});
    $("#slider4").slider({orientation: "vertical", min: 0, max: 500, range: true, values: [150, 350]});
    if ($("#price_rage").length > 0) {
        $("#price_rage").slider({range: true, min: 0, max: 3e3, values: [1e3, 2e3], slide: function (e, t) {
            $("#price_amount").html("$" + t.values[0] + " - $" + t.values[1])
        }});
        $("#price_amount").html("$" + $("#price_rage").slider("values", 0) + " - $" + $("#price_rage").slider("values", 1))
    }
    $("#spinner").spinner();
    $("#spinner2").spinner({step: .1});
...
$(document).ready(function () {
    function d(e, t) {
        return"<div style='text-shadow: 1px 2px 1px rgba(0,0,0,0.2);font - size:11px;text - align:center;padding:2px;color: #FFF;        line - height:13px;'>"+e+"<br/>"+Math.round(t.percent)+"%</div>"}function v(e,t,n){$(' < div class= "ftooltip" > '+n+"</div>").css({position:"absolute","z-index":"10",display:"none",top:t-20,left:e,padding:"3px","background-color":"rgba(0,0,0,0.5)","font-size":"11px","border-radius":"3px",color:"#FFF"}).appendTo("body").fadeIn(200)}if($("#dash_chart_1").length>0){var e=[[1,25],[2,28],[3,22],[4,18],[5,30],[6,18],[7,14]];
    ...
    if ($("#dash_chart_2").length > 0) {
        ...
        var i = $.plot($("#dash_chart_2"), [
            {data: n, label: "Search Traffic"},
            {data: r, label: "Referal Traffic"}
    ...
    if ($("#chart_bar_1").length > 0) {
        ...
        var c = $.plot($("#chart_bar_1"), [
            ...
            {data: r, label: "Data 2"},
            {data: l, label: "Data 3"}
        ], {series: {stack: 0, bars: {show: true, barWidth: .8, align: "center"}}, grid: {hoverable: true, clickable: true}})
    }
    ...
    if ($("#chart_pie_1").length > 0) {
        var e = [];
        e[0] = {label: "Data 1", data: 40};
        e[1] = {label: "Data 2", data: 30};
        e[2] = {label: "Data 3", data: 30};
        $.plot("#chart_pie_1", e, {series: {pie: {radius: 1, show: true, label: {show: true, radius: 2 / 3, formatter: d, threshold: .1}}}, legend: {show: false}})
    }
...
$(document).ready(function () {
    if ($("#google_kyiv_map").length > 0) {
        ...
        var t = {zoom: 10, center: e, mapTypeId: google.maps.MapTypeId.ROADMAP};
        var n = new google.maps.Map(document.getElementById("google_kyiv_map"), t);
        var r = new google.maps.LatLng(50.43, 30.6);
        var i = new google.maps.Marker({position: r, map: n, title: "Office title"})
    }
    if ($("#google_world_map").length > 0) {
        var s = new google.maps.LatLng(50, 0);
        var o = {zoom: 1, center: s, mapTypeId: google.maps.MapTypeId.ROADMAP};
        var u = new google.maps.Map(document.getElementById("google_world_map"), o);
        var a = new google.maps.LatLng(50, 10);
        var f = {zoom: 3, center: a, mapTypeId: google.maps.MapTypeId.ROADMAP};
        var l = new google.maps.Map(document.getElementById("google_eu_map"), f);
        var c = new google.maps.LatLng(50.43, 30.6);
        var h = {zoom: 8, center: c, mapTypeId: google.maps.MapTypeId.ROADMAP};
        var p = new google.maps.Map(document.getElementById("google_ptm_map"), h);
        var d = new google.maps.LatLng(50.37, 30.65);
        var v = new google.maps.Marker({position: d, map: p, title: "Marker 1"});
        d = new google.maps.LatLng(50.5, 30.55);
        v = new google.maps.Marker({position: d, map: p, title: "Marker 2"});
        d = new google.maps.LatLng(50.8, 30.55);
        v = new google.maps.Marker({position: d, map: p, title: "Marker 3"});
        function m() {
            var e = new google.maps.Map(document.getElementById("google_search_map"), {mapTypeId: google.maps.MapTypeId.ROADMAP});
            ...
            google.maps.event.addListener(r, "places_changed", function () {
                ...
                for (var n = 0, u;
                     u = t[n];
                     n++) {
                    var a = {url: u.icon, size: new google.maps.Size(71, 71), origin: new google.maps.Point(0, 0), anchor: new google.maps.Point(17, 34), scaledSize: new google.maps.Size(25, 25)};
                    var s = new google.maps.Marker({map: e, icon: a, title: u.name, position: u.geometry.location});
                    i.push(s);
